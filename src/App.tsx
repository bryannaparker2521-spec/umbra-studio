...
async function sendDirectMessage(){if(!session||!messageRecipientId||!messageBody.trim()||messagesBusy)return;const body=messageBody.trim();setMessagesBusy(true);setMessagesError("");try{const {error}=await supabase.rpc("studio_send_direct_message",{target_user_id:messageRecipientId,message_body:body});if(error)throw error;setMessageBody("");await loadDirectMessages();}catch(f){setMessagesError(f instanceof Error?f.message:"Message could not be sent.");}finally{setMessagesBusy(false);}}
