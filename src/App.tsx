...
if (loading) {
return ( <main className="studio-shell"> <div className="studio-card"> <div className="moon">â˜¾</div>
...
if (!session && publicBrowse) {
 ...
 return (<main className="public-encyclopedia-page"><header className="studio-header"><div className="brand"><div className="brand-moon">â˜¾</div><div><p className="header-eyebrow">UMBRA CONNECT</p><h2>{publicBrowseSettings?.title||"The Umbral World"}</h2></div></div><button className="back-button" onClick={()=>setPublicBrowse(false)}>Studio Sign In</button></header><section className="public-encyclopedia-shell"><div className="public-encyclopedia-hero" style={publicBrowseSettings?.hero_image_url?{backgroundImage:`linear-gradient(rgba(7,4,12,.65),rgba(7,4,12,.96)),url(${publicBrowseSettings.hero_image_url})`}:undefined}><p className="eyebrow">PUBLIC UMBRA ENCYCLOPEDIA â€¢ V2</p><h1>{publicBrowseSettings?.title||"The Umbral World"}</h1><h3>{publicBrowseSettings?.subtitle||"Explore published canon from Umbra Connect."}</h3><p>{publicBrowseSettings?.introduction||"Discover the people, places, powers, histories, and artifacts of the Umbral World."}</p><input type="search" value={publicBrowseSearch} onChange={e=>setPublicBrowseSearch(e.target.value)} placeholder="Search published characters, lore, places, and history..."/></div><nav className="public-v9-tabs">{(["lore","characters","codex","locations","timeline"] as const).map(t=><button className={publicBrowseTab===t?"active":""} key={t} onClick={()=>setPublicBrowseTab(t)}>{t}</button>)}</nav>
 {publicBrowseTab==="lore"&&<div className="v8-encyclopedia-grid public-grid">{shown.map(r=><article key={r.id}>{r.image_url&&<img src={r.image_url} alt=""/>}<span>{publicBrowseTypes.find(t=>t.id===r.record_type_id)?.name||"Lore"}</span><h3>{r.name}</h3>{r.subtitle&&<strong>{r.subtitle}</strong>}<p>{r.summary||""}</p><small>{r.record_code} â€¢ {(r.canon_status||"").replace(/_/g," ")}</small>{Object.entries(r.details||{}).slice(0,6).map(([k,v])=><details key={k}><summary>{k}</summary><p>{typeof v==="string"?v:JSON.stringify(v)}</p></details>)}</article>)}</div>}
 {publicBrowseTab==="characters"&&<div className="v8-encyclopedia-grid public-grid">{shownChars.map(c=><article key={c.id}>{c.portrait_url&&<img src={c.portrait_url} alt=""/>}<span>Character</span><h3>{c.name}</h3><strong>{[c.identity?.race,c.identity?.homeland].filter(Boolean).join(" â€¢ ")}</strong><p>{c.identity?.summary||c.origin_lore?.backstory||""}</p>{c.abilities?.primaryAbilities&&<details><summary>Abilities</summary><p>{c.abilities.primaryAbilities}</p></details>}</article>)}</div>}
 ...
 {publicBrowseTab==="locations"&&<div className="v8-encyclopedia-grid public-grid">{shownLoc.map(l=><article key={l.id}>{l.image_url&&<img src={l.image_url} alt=""/>}<span>{l.location_type}</span><h3>{l.name}</h3><p>{l.description||""}</p>{l.tags?.length?<small>{l.tags.join(" â€¢ ")}</small>:null}</article>)}</div>}
 {publicBrowseTab==="timeline"&&<div className="public-v9-timeline">{shownTime.map(e=><article key={e.id}>{e.image_url&&<img src={e.image_url} alt=""/>}<div><span>{e.era||"Era unknown"}{e.display_date?` â€¢ ${e.display_date}`:""}</span><h3>{e.title}</h3><p>{e.description||""}</p></div></article>)}</div>}
...
if (!session) {
return ( <main className="studio-shell"> <div className="studio-card login-card"> <div className="moon">â˜¾</div>
...
return (<main className="studio-shell"><div className="studio-card"><div className="moon">â˜¾</div><p className="eyebrow">UMBRA CONNECT</p><h1>Umbra Studio</h1><p className="subtitle">Checking Studio authorization...</p><div className="divider"/><p className="status">Studio access is invitation-only.</p></div></main>);
}

if (!studioAccessRole) {
return (<main className="studio-shell"><div className="studio-card access-denied-card"><div className="moon">â˜¾</div><p className="eyebrow">PRIVATE CREATIVE DATABASE</p><h1>Access Restricted</h1><p className="subtitle">Your Umbra Connect account is valid, but it has not been authorized for Umbra Studio.</p><div className="divider"/><p className="status">A Primary Admin must add your account as Primary Admin, Admin, or Editor before Studio data can be accessed.</p><button className="sign-out-button access-signout" onClick={()=>void handleSignOut()}>Sign Out</button></div></main>);
}
...
return (
  <main className="dashboard-shell">
    ...
    <header className="studio-header">
      <div className="brand">
        <div className="brand-moon">â˜¾</div>
        <div>
    ...
    <section className="dashboard-content">
      ...
      {loadingCharacters ? (
        ...
        <div className="studio-footer-card">
          <div>
            <span className="connected-check">âœ¦</span>
              ...
              <article className="saved-character-card" key={saved.id}>
                <div className="saved-character-image">
                  {portrait ? (
                    ...
                    <div className="saved-character-placeholder">â˜¾</div>
                  )}
                ...
                <div className="saved-character-body">
                  ...
                  <p>
                    {identity.summary ||
                      [identity.homeland, identity.affiliation].filter(Boolean).join(" â€¢ ") ||
                  ...
                  <div className="saved-character-actions">
                    ...
                    >
                      {saved.is_complete ? "View / Edit Character" : "Continue Editing â†’"}
                    </button>
                    {saved.is_complete && (
                      <button
                        type="button"
...
return (
  <main className="dashboard-shell">
    ...
    `}</style>
    <header className="studio-header">
      <div className="brand"><div className="brand-moon">â˜¾</div><div><p className="header-eyebrow">UMBRA CONNECT</p><h2>Umbra Studio</h2></div></div>
      <div className="account-area"><button type="button" className="sign-out-button" onClick={returnToDashboard}>â† Back to Studio</button></div>
    </header>
    <section className="dashboard-content">
      <div className="welcome-section">
      ...
      {loadingLibrary ? (
        ...
        <div className="studio-footer-card"><div><span className="connected-check">âœ¦</span><strong>The Library is waiting for its first published soul</strong></div><p>Characters appear here only after their creator publishes them.</p></div>
      ) : filteredLibraryCharacters.length === 0 ? (
        <div className="studio-footer-card"><div><span className="connected-check">âœ¦</span><strong>No characters match those filters</strong></div><p>Try clearing a search term or filter.</p></div>
      ) : (
            ...
            return (
              <article className="library-card" key={saved.id}>
                <div className="library-image">
                  {portrait ? <img src={portrait} alt={`${saved.name} portrait`} /> : <div className="library-placeholder">â˜¾</div>}
                ...
                <div className="library-body">
                  ...
                  <h3>{saved.name || "Unnamed Character"}</h3>
                  {identity.alias && <strong>{identity.alias}</strong>}
                  <p>{identity.summary || [identity.homeland, identity.affiliation].filter(Boolean).join(" â€¢ ") || "A completed character of the Umbral World."}</p>
...
<header className="studio-header"><button className="brand-button" onClick={returnToDashboard}><div className="brand-moon">â˜¾</div><div className="brand-button-copy"><span className="header-eyebrow">UMBRA CONNECT</span><strong>World Explorer</strong></div></button><button className="sign-out-button" onClick={returnToDashboard}>â† Back to Studio</button></header>
<section className="explorer-shell">
<div className="explorer-heading"><div><p className="eyebrow">MAP â€¢ HISTORY â€¢ LORE</p><h1>World Explorer</h1><p>Navigate, edit, archive, restore, and chronicle the Umbral World from one workspace.</p></div><input className="explorer-search" type="search" value={explorerSearch} onChange={e=>setExplorerSearch(e.target.value)} placeholder="Search locations, eras, tags, lore..."/></div>
...
return (
  <main className="dashboard-shell codex-page">
    ...
    `}</style>
    <header className="studio-header"><div className="brand"><div className="brand-moon">â˜¾</div><div><p className="header-eyebrow">UMBRA CONNECT</p><h2>Umbra Studio</h2></div></div><div className="account-area"><button type="button" className="sign-out-button" onClick={returnToDashboard}>â† Back to Studio</button></div></header>
...
const ConnectionNode = ({ link }: { link: CharacterRelationship }) => {
  ...
  return (
    <button type="button" className={`connection-node relation-${link.relationship_type}`} onClick={() => void moveConnectionCenter(target)}>
      <div className="connection-node-image">{image ? <img src={image} alt={`${target.name} portrait`} /> : <span>â˜¾</span>}</div>
...
return (
  <main className="dashboard-shell connections-page">
    ...
    <header className="studio-header">
      <div className="brand"><div className="brand-moon">â˜¾</div><div><p className="header-eyebrow">UMBRA CONNECT</p><h2>Umbra Studio</h2></div></div>
      <div className="account-area"><button type="button" className="sign-out-button" onClick={returnFromConnections}>â† Back to Profile</button></div>
    </header>
    <section className="connections-content">
      ...
      {loadingConnections ? <div className="connections-empty"><strong>Tracing connections...</strong><p>Following the bonds surrounding this character.</p></div> : connectionView === "family" ? (
        ...
        <div className="family-tree">
          ...
          <div className="tree-level">
            ...
            <div className="tree-row">
              ...
              <div className="connection-center"><div className="connection-center-image">{centerPortrait ? <img src={centerPortrait} alt={`${connectionCenter.name} portrait`} /> : "â˜¾"}</div><div><span className="creator-kicker">CENTER OF TREE</span><h2>{connectionCenter.name}</h2><p>{connectionCenter.identity?.alias || connectionCenter.identity?.race || "Umbral Character"}</p></div></div>
            </div>
          </div>
        ...
        <div className="all-connections-map">
          ...
          <div className="connection-center all-map-center"><div className="connection-center-image">{centerPortrait ? <img src={centerPortrait} alt={`${connectionCenter.name} portrait`} /> : "â˜¾"}</div><div><span className="creator-kicker">CENTER OF MAP</span><h2>{connectionCenter.name}</h2><p>{connectionCenter.identity?.alias || connectionCenter.identity?.race || "Umbral Character"}</p></div></div>
        </div>
      )}
...
return (
  <main className="dashboard-shell profile-page">
    ...
    <header className="studio-header">
      <div className="brand"><div className="brand-moon">â˜¾</div><div><p className="header-eyebrow">UMBRA CONNECT</p><h2>Umbra Studio</h2></div></div>
      <div className="account-area"><button type="button" className="sign-out-button" onClick={returnFromProfile}>â† Back</button></div>
    </header>
    <section className="profile-hero">
      <div className="profile-hero-bg">{portrait && <img src={portrait} alt="" />}</div>
      <div className="profile-hero-content">
        <div className="profile-portrait">{portrait ? <img src={portrait} alt={`${saved.name} portrait`} /> : "â˜¾"}</div>
    ...
    <div className="profile-content">
      <ProfileSection title="Identity" symbol="âœ¦" items={identityItems} />
      <ProfileSection title="Appearance" symbol="â—ˆ" items={appearanceItems} />
      <ProfileSection title="Origin & Lore" symbol="â˜¾" items={loreItems} />
      <ProfileSection title="Abilities & Combat" symbol="âš”" items={abilityItems} />
      {connectedRelationships.length > 0 && <section className="profile-section"><div className="profile-section-title"><span>â™™</span><h2>Character Connections</h2></div><div className="profile-media-grid">{connectedRelationships.map((link) => { const target=link.target; if(!target) return null; const image=target.portrait_url || target.media?.portraitUrl || ""; return <button type="button" className="profile-media-card" style={{textAlign:"left",cursor:"pointer",color:"inherit"}} key={link.id} onClick={() => void openConnectedCharacterProfile(target)}>{image ? <img src={image} alt={`${target.name} portrait`} /> : <div style={{height:300,display:"grid",placeItems:"center",fontSize:64,color:"#e5bd57"}}>â˜¾</div>}<span style={{textTransform:"capitalize"}}>{target.name} â€¢ {link.relationship_type}</span></button>})}</div></section>}
      <ProfileSection title="Written Relationships" symbol="â™™" items={relationshipItems} />
      {(mediaItems.length > 0 || galleryImages.length > 0 || media.galleryUrl || media.mediaNotes) && <section className="profile-section"><div className="profile-section-title"><span>â–£</span><h2>Media & References</h2></div>{(mediaItems.length > 0 || galleryImages.length > 0) && <div className="profile-media-grid">{mediaItems.map((item) => <a className="profile-media-card" href={item.url} target="_blank" rel="noreferrer" key={item.label}><img src={item.url} alt={`${saved.name} ${item.label}`} /><span>{item.label}</span></a>)}{galleryImages.map((url: string, index: number) => <a className="profile-media-card" href={url} target="_blank" rel="noreferrer" key={url}><img src={url} alt={`${saved.name} gallery ${index + 1}`} /><span>Gallery Image {index + 1}</span></a>)}</div>}{media.galleryUrl && <a className="profile-gallery-link" href={media.galleryUrl} target="_blank" rel="noreferrer">Open additional gallery / media â†’</a>}{media.mediaNotes && <p className="profile-media-notes">{media.mediaNotes}</p>}</section>}
    </div>
  </main>
);
}
...
const renderIdentityStep = () => (
  <section className="creator-form-card">
    <div className="form-section-heading">
      <span className="form-section-icon">âœ¦</span>
      <div>
    ...
    <div className="creator-form-actions">
      ...
      <button type="button" className="primary-action" onClick={() => saveCharacter(2)} disabled={savingCharacter}>
        {savingCharacter ? "Saving..." : "Save & Continue â†’"}
      </button>
    </div>
  </section>
);
  ...
  <section className="creator-form-card">
    ...
    <div className="connection-builder">
      ...
      <div className="connection-controls">
        ...
        <select value={relationshipTargetId} onChange={(e) => setRelationshipTargetId(e.target.value)}>
          ...
          {relationshipOptions.map((item) => <option key={item.id} value={item.id}>{item.name || "Unnamed Character"}{item.identity?.alias ? ` â€” ${item.identity.alias}` : ""}</option>)}
        </select>
      ...
      {connectedRelationships.length > 0 && <div className="connection-list">{connectedRelationships.map((link) => { const target=link.target; const image=target?.portrait_url || target?.media?.portraitUrl || ""; return <div className="connection-chip" key={link.id}>{image ? <img src={image} alt="" /> : <div className="connection-avatar">â˜¾</div>}<div className="connection-chip-copy"><strong>{target?.name || "Character"}</strong><span>{link.relationship_type}</span></div><button type="button" className="connection-remove" title="Remove connection" onClick={() => void removeConnectedRelationship(link)}>Ã—</button></div>})}</div>}
    </div>
    ...
    <div className="creator-form-grid">
      ...
      <label className="creator-field">
        ...
        <textarea
          rows={5}
          placeholder="Teachers, masters, apprentices, protÃ©gÃ©s..."
  ...
  <section className="creator-form-card">
    ...
    <div className="media-upload-grid">
      {[
        ...
        <div className="media-upload-card" key={item.kind}>
          ...
          <p className="upload-help">{uploadingMedia === item.kind ? "Uploading..." : "PNG, JPG, WEBP or GIF â€¢ max 10 MB"}</p>
        </div>
      ))}
    </div>
    ...
    <div className="creator-form-grid">
      ...
      <div className="creator-field full-width">
        ...
        {character.galleryUrls.length > 0 && <div className="media-gallery-editor">{character.galleryUrls.map((url, index) => <div className="media-gallery-item" key={url}><img src={url} alt={`Gallery ${index + 1}`} /><button type="button" title="Remove from profile" onClick={() => removeGalleryImage(url)}>Ã—</button></div>)}</div>}
      </div>
    ...
    <div className="creator-form-actions">
      <button type="button" className="secondary-action" onClick={() => goToCreatorStep(5)}>â† Back</button>
      <button type="button" className="primary-action" onClick={() => saveCharacter(6, true)} disabled={savingCharacter || uploadingMedia !== null}>
        {savingCharacter ? "Saving..." : uploadingMedia ? "Uploading..." : "Finish Character âœ“"}
      </button>
    </div>
  </section>
);
...
return (
  <main className="creator-page">
    <header className="creator-topbar">
      ...
      >
        <span className="brand-moon">â˜¾</span>
          ...
          <aside className="character-preview-card">
            <div className="preview-image-placeholder">
              {character.portraitUrl.trim() ? (
                ...
                <>
                  <div className="preview-moon">â˜¾</div>
              ...
              <p>
                {character.summary.trim() ||
                  (creatorStep === 2 && character.race.trim()
                    ? `${character.race}${character.homeland.trim() ? ` â€¢ ${character.homeland}` : ""}`
...
if(page==="production"){
 ...
 return <main className="dashboard-shell production-v9-page"><header className="studio-header"><button className="brand-button" onClick={()=>setPage("dashboard")}><div className="brand-moon">â˜¾</div><div className="brand-button-copy"><span className="header-eyebrow">UMBRA CONNECT</span><strong>Umbra Studio</strong></div></button><div className="account-area"><span className="admin-role-pill">V9 MEGA</span><button className="back-button" onClick={()=>setPage("dashboard")}>Dashboard</button></div></header><section className="production-v9-shell"><div className="production-v9-hero"><div><p className="eyebrow">STORY â€¢ COLLABORATION â€¢ WORLD INTELLIGENCE</p><h1>Story Production Center</h1><p>Plan stories without duplicating your lore. Connect projects, arcs, scenes, plot beats, characters, locations, canon records, reviews, assignments, and collaborator activity.</p></div><button className="secondary-action" onClick={()=>void loadV9Production()}>{productionBusy?"Refreshing...":"Refresh Production"}</button></div>{productionError&&<p className="login-error">{productionError}</p>}
...
â˜¾ </div>
  ...
  <section className="dashboard-content">
    ...
    <section className="studio-command-center">
      <div className="command-search"><span>âŒ•</span><input type="search" value={studioSearch} onChange={e=>setStudioSearch(e.target.value)} placeholder="Search your Studio â€” characters, Codex, locations, timeline..."/></div>
    ...
    <div className="dashboard-grid">
      ...
      <button type="button" className="dashboard-card" onClick={() => void openWorldExplorer("map")}>
        <div className="card-icon">âœ§</div><div><span className="card-label">EXPLORE & CHRONICLE</span><h3>World Explorer</h3><p>Open the interactive map, nested locations, historical timeline, tags, and favorites.</p></div><span className="card-arrow">â†’</span>
      </button>


      <button type="button" className="dashboard-card database-dashboard-card" onClick={() => void openWorldDatabase()}><div className="card-icon">â–¦</div><div><span className="card-label">CANON â€¢ DATABASE â€¢ PUBLISHING</span><h3>World Database</h3><p>Control canon, continuity, public encyclopedia records, expanded lore, collections, links, media, imports, exports, and backups.</p></div><span className="card-arrow">â†’</span></button>

      <button type="button" className="dashboard-card production-dashboard-card" onClick={() => void openProduction()}>
        <div className="card-icon">âœ¦</div>
        <div>
          <span className="card-label">STORY â€¢ PLANNING â€¢ COLLABORATION</span>
      ...
      <button type="button" className="dashboard-card v101-messages-card" onClick={() => void openMessages()}><div className="card-icon">âœ‰</div><div><span className="card-label">COLLABORATOR â€¢ DIRECT MESSAGES</span><h3>Studio Messages</h3><p>Private conversations with your authorized Studio collaborators, with unread and read status.</p></div><span className="card-arrow">â†’</span></button>

      <button type="button" className="dashboard-card v101-transfer-card" onClick={() => void openTransferCenter()}><div className="card-icon">â‡©</div><div><span className="card-label">BACKUP â€¢ TRANSFER â€¢ SETUP</span><h3>Backup & Transfer Center</h3><p>Download complete Studio backups, create cloud snapshots, validate backup files, and set up another admin computer.</p></div><span className="card-arrow">â†’</span></button>

      <button type="button" className="dashboard-card v10-settings-card" onClick={() => void openStudioSettings()}><div className="card-icon">âš™</div><div><span className="card-label">STUDIO 1.0 â€¢ PREFERENCES</span><h3>Studio Settings</h3><p>Control autosave, canon and spoiler defaults, collaborator presence, dashboard preferences, and Studio identity.</p></div><span className="card-arrow">â†’</span></button>

      <button type="button" className="dashboard-card admin-dashboard-card" onClick={() => void openAdminCenter()}><div className="card-icon">âš™</div><div><span className="card-label">COLLABORATE & MANAGE</span><h3>Admin Center</h3><p>Manage collaborator names, login timestamps, presence history, editorial workflow, revisions, private notes, and the full change trail.</p></div><span className="card-arrow">â†’</span></button>
    </div>
