...
const databaseGuide:Record<string,string>={records:"Create and edit structured lore records. Use this for detailed database entries rather than character profiles.",canon:"Manage canon status and retcon history so the team knows what is official, draft, replaced, or deprecated.",continuity:"Track contradictions and unresolved lore or story conflicts before they become canon problems.",encyclopedia:"Control the public-facing lore encyclopedia and what approved information can be shown outside the private Studio.",media:"Upload and organize reusable Studio images, maps, references, documents, and production files.",import:"Bring existing character or lore material into Studio for review before saving.",collections:"Group related database records into reusable collections.",tags:"Create labels that make records easier to organize and find.",links:"Connect records to other Studio entities without duplicating information.",bulk:"Apply workflow or archive actions to multiple selected records at once.",health:"Check database completeness, missing information, and structural health.",revisions:"Review database-specific snapshots and restore an earlier record when needed.",duplicates:"Find likely duplicate records before they split your canon.",templates:"Create reusable field structures for consistent lore records.",backup:"Export or snapshot Studio data before major changes."};
...
async function uploadRecordHeroImage(file:File){
 if(!session)return;
 if(!file.type.startsWith("image/")){setDatabaseError("Choose an image file.");return;}
...
<nav className="explorer-tabs">{([['map','World Map'],['locations','Locations'],['timeline','Timeline'],['favorites','Favorites'],['archive',`Archive (${archivedLocations.length+archivedEvents.length})`]] as const).map(([id,label])=><button key={id} className={explorerTab===id?'active':''} onClick={()=>setExplorerTab(id)}>{label}</button>)}</nav><StudioGuide title="World Explorer">{explorerGuide[explorerTab]}</StudioGuide>
